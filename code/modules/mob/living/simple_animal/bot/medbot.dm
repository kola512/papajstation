...
/mob/living/simple_animal/bot/medbot/process_scan(mob/living/carbon/human/H)
	...
	if(assess_patient(H))
		...
		if((last_newpatient_speak + 300) < world.time) //Don't spam these messages!
			var/list/messagevoice = list("Hey, [H.name]! Czekaj, juz ode." = 'sound/voice/medbot/coming.ogg',"Czekaj [H.name]! Chce ci pomoc!" = 'sound/voice/medbot/help.ogg',"[H.name], wygladasz na rannego!" = 'sound/voice/medbot/injured.ogg')
...
/mob/living/simple_animal/bot/medbot/handle_automated_action()
	...
	if(QDELETED(patient))
		if(!shut_up && prob(1))
			var/list/messagevoice = list("Radar, wloz maske!" = 'sound/voice/medbot/radar.ogg',"Zawsze jest haczyk, a ja jestem najlepszy." = 'sound/voice/medbot/catch.ogg',"Wiedzialem o tym, powinienem byc chirurgiem plastyczny." = 'sound/voice/medbot/surgeon.ogg',"Co to za ambulatorium? Wszyscy padaja jak muchy." = 'sound/voice/medbot/flies.ogg',"Przepyszne!" = 'sound/voice/medbot/delicious.ogg')
			var/message = pick(messagevoice)
			speak(message)
			playsound(loc, messagevoice[message], 50, 0)
...
/mob/living/simple_animal/bot/medbot/proc/medicate_patient(mob/living/carbon/C)
	...
	if(C.stat == DEAD || (C.has_trait(TRAIT_FAKEDEATH)))
		var/list/messagevoice = list("Stoj! Zostan ze mna" = 'sound/voice/medbot/no.ogg',"Zyj kurwo!" = 'sound/voice/medbot/live.ogg',"Ja... nigdy wczesniej nie stracilem pacjenta. Nie dzisiaj, oczywiscie" = 'sound/voice/medbot/lost.ogg')
		var/message = pick(messagevoice)
		speak(message)
		playsound(loc, messagevoice[message], 50, 0)
		oldpatient = patient
	...
	if(!reagent_id) //If they don't need any of that they're probably cured!
		...
		var/list/messagevoice = list("Wszystko zalatane!" = 'sound/voice/medbot/patchedup.ogg',"Jablko dziennie powstrzymuje mnie." = 'sound/voice/medbot/apple.ogg',"Szybkiego powrotu do zdrowia!" = 'sound/voice/medbot/feelbetter.ogg')
		var/message = pick(messagevoice)
		speak(message)
		playsound(loc, messagevoice[message], 50, 0)
		bot_reset()
		...
		if(!emagged && check_overdose(patient,reagent_id,injection_amount))
			soft_reset()
			return
		C.visible_message("<span class='danger'>[src] probuje zaszczepic [patient]!</span>", \
			"<span class='userdanger'>[src] probuje cie zaszczepic!</span>")
		...
		if(do_mob(src, patient, 30))	//Is C == patient? This is so confusing
			if((get_dist(src, patient) <= 1) && (on) && assess_patient(patient))
				...
				C.visible_message("<span class='danger'>[src] szczepi [patient] szczepionka!</span>", \
					"<span class='userdanger'>[src] szczepi cie szczepionka!</span>")
			else
				failed = TRUE
		else
			failed = TRUE
		...
		if(failed)
			visible_message("[src] wsuwa igle.")
		update_icon()
		soft_reset()
		return
...
/mob/living/simple_animal/bot/medbot/explode()
	on = FALSE
	visible_message("<span class='boldannounce'>[src] rozpierdala sie!</span>")
